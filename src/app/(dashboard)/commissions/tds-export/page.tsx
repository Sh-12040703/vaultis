...
export default function TDSExportPage() {
  ...
  function downloadCSV() {
    ...
    csvRows.push([`Total Commission Earned: ₹${data.totalExpected.toFixed(2)}`])
    csvRows.push([`Total Commission Received: ₹${data.totalReceived.toFixed(2)}`])
    csvRows.push([`Total TDS Deducted (Sec 194D): ₹${data.totalTDS.toFixed(2)}`])
    csvRows.push([`Short Paid: ₹${(data.totalExpected - data.totalReceived).toFixed(2)}`])
    csvRows.push([''])
    ...
    <div className="p-8 max-w-4xl space-y-6">
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-xl p-5 flex items-center gap-4 flex-wrap">
        ...
        <button
          ...
          className={`flex items-center gap-2 font-semibold px-5 py-2.5 rounded-lg transition-colors text-sm ${
